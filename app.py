...
def load_demo_data():
    """Loads static CSV for Demo Mode (Local or S3)"""
    file_path = "dashboard_feed.csv"
    
    # Try S3 Download if missing
    if not os.path.exists(file_path):
        try:
            ...
            if bucket and aws_key:
                ...
                st.toast("Downloaded Demo Data from S3", icon="⬇️")
    ...
    try:
        ...
        st.error("Demo data not found (Local or S3). Please run snapshot_generator.py first.")
        return pd.DataFrame()
...
def main():
    st.title("🛡️ Enterprise Data Quality Monitor")
    ...
    if mode == "Demo (Static Snapshot)":
        ...
        with st.spinner("Connecting to Snowflake..."):
            try:
                ...
                st.toast("Connected to Snowflake Live!", icon="🟢")
            except Exception as e:
